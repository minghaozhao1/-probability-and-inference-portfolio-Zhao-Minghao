...
while ( p<=0.8 ) {
  x=x+2
  p=pnbinom((x-1)/2,(x+1)/2,0.55+0.001*i)
...
```{r}
pnbinom(3,4,0.55)*0.5/(pnbinom(3,4,0.55)*0.5+pnbinom(3,4,0.45)*0.5)
```
use the bayes rule, we can find the equation to calculate the P(PB=0.55|Braves win World Series in 7 games). Since we assume the probability of winning have 0.5 chance for each value, we applied half of their probability when we are calculating the P(Braves win World Series in 7 games). The result is 0.6082878
